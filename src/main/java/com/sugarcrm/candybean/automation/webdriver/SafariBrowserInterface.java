...
public class SafariBrowserInterface extends WebDriverInterface {
	...
	public void start() throws CandybeanException {
		...
		validateChromeDriverExist(safariDriverPath);
		...
		if("true".equals(candybean.config.getPathValue("parallel.enabled"))) {
			String originalChromePath = safariDriverPath;
			...
			if(!new File(safariDriverPath).exists()) {
				try {
					FileUtils.copyFile(new File(originalChromePath), new File(safariDriverPath));
	...
	private void validateChromeDriverExist(String safariDriverPath) throws CandybeanException {
